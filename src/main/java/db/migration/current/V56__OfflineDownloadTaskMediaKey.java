...
/** V56: associate an offline task with a stable media identity for acquire de-duplication. */
public class V56__OfflineDownloadTaskMediaKey extends BaseJavaMigration {
    @Override
    public void migrate(Context context) throws Exception {
        Connection connection = context.getConnection();
        String table = findTable(connection, "offline_download_task");
        if (table != null && findColumn(connection, table, "media_key") == null) {
            try (Statement statement = connection.createStatement()) {
                statement.execute("ALTER TABLE " + table + " ADD COLUMN media_key VARCHAR(128)");
            }
        }
    }
