...
public class PianDanSubscriptionService {
    ...
    public record PianDanEntry(String vodId, String name, Integer season, Integer year, Integer doubanId) {
    }
    ...
    /** 豆瓣片单条目自动绑元数据:db:{id} 条目直绑零消歧零网络;无 id 的 s: 条目回落本地豆瓣库
     *  精确名匹配(零网络,年份可消歧同名翻拍);再回落 suggest 名称精确匹配。
     *  后两级的同名消歧口径一致:名称相等的结果**恰好一个**才绑 —— 同名翻拍多条且无年份可用时,suggest 也无法消歧
     *  (片单条目无年份),取首条必赌错一半,不如不绑回落纯标题订阅,交给巡检名称桥接(有季号/年份上下文)消歧。 */
    private void bindDoubanMeta(MediaSubscriptionRequest request, PianDanEntry entry) {
        if (entry.doubanId() != null) {
            request.setDoubanId(entry.doubanId());
            request.setMetaProvider("douban");
            request.setMetaId(String.valueOf(entry.doubanId()));
            return;
        }
        Integer localId = mediaSubscriptionService.localDoubanId(entry.name(), entry.year());
        if (localId != null) {
        ...
        try {
            Object items = mediaSubscriptionService.metaSearch("douban", entry.name()).get("items");
            if (!(items instanceof List<?> list)) {
                return;
            }
            Set<String> matchedIds = new HashSet<>();
            for (Object candidate : list) {
                if (candidate instanceof MetadataSearchItem item
                        && "douban".equals(item.getProvider()) && entry.name().equals(item.getName())
            ...
            log.debug("douban meta bind for pian-dan entry {} failed: {}", entry.name(), e.getMessage());
        }
    }

    /** 片单条目载荷 → (vodId, 剧名, 季号?, 年份?, 豆瓣id?):详情页装配 play id 时已内嵌剧名与季号({vodId}|{名}|{季}),
     *  订阅/取消零网络;多季剧的季号让订阅精确到季(create 对显式 >1 的季号不覆盖)。
     *  无内嵌名的条目现拉一次标题兜底(TMDB zh-CN / db: 按 id 本地直取或 rexxar);s: 条目标题在 id 里、年份取后缀。 */
    public PianDanEntry pianDanEntry(String payload) {
        ...
        if (vodId.startsWith(PianDanService.DOUBAN_SUBJECT_PREFIX)) {
            try {
                doubanId = Integer.valueOf(vodId.substring(PianDanService.DOUBAN_SUBJECT_PREFIX.length()));
            } catch (NumberFormatException e) {
                throw new BadRequestException("无效的片单条目: " + vodId);
            }
        } else if (StringUtils.isBlank(name) && vodId.startsWith("tmdb:")) {
            name = tmdbMeta(vodId).getVod_name();
        }
        if (StringUtils.isBlank(name)) {
            if (doubanId != null) {
                // db: 条目标题不在 id 里:本地库 id 直取,未收录回落 rexxar 在线解析
                MovieDetail meta = mediaSubscriptionService.localDoubanDetailById(doubanId);
                if (meta == null) {
                    meta = pianDanService.doubanSubjectDetail(doubanId);
                }
                if (meta == null || StringUtils.isBlank(meta.getVod_name())) {
                    throw new BadRequestException("片单条目信息获取失败: " + vodId);
                }
                name = meta.getVod_name();
        ...
        return new PianDanEntry(vodId, name, season, year, doubanId);
    }
